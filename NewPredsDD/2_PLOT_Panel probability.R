...
for(i in 1:100){
  ...
  polygon( x.polygon , y.polygon , col = sigColorP, border = NA )
  
  # Add posterior means
  lines(X3, meanProbGroup[,,i],lwd=1, col='black',lty=1)

  text(0.1,0.9,round(probs1[i],2),cex=0.8, font=2)
...
mtext(y.label, line = 1.5, side = 2, cex = size.text,outer=T, adj=0.9)
mtext(x.label, line = -18.2, side = 1, cex = size.text, outer=T)
# 
# box()
