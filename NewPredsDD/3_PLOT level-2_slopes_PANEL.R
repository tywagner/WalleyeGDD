...
for(i in 1:length(mean.beta)) { 
  u.alpha[i] <- quantile(out$BUGSoutput$sims.list$BB[,i,2],probs=c(0.95) )
  l.alpha[i] <- quantile(out$BUGSoutput$sims.list$BB[,i,2],probs=c(0.05) )
}
########
...
upper.CIAa <- apply(est.lineAa, 2, quantile, probs=c(0.95) )
lower.CIAa <- apply(est.lineAa, 2, quantile, probs=c(0.05) )

## Grab 90% CIs for beta's
u.alphaa <- numeric(length(mean.beta) )
l.alphaa <- numeric(length(mean.beta) )
for(i in 1:length(mean.beta)) { 
  u.alphaa[i] <- quantile(out$BUGSoutput$sims.list$BB[,i,2],probs=c(0.95) )
  l.alphaa[i] <- quantile(out$BUGSoutput$sims.list$BB[,i,2],probs=c(0.05) )
}
########
...
upper.CIAab <- apply(est.lineAab, 2, quantile, probs=c(0.95) )
lower.CIAab <- apply(est.lineAab, 2, quantile, probs=c(0.05) )

## Grab 90% CIs for beta's
u.alphaab <- numeric(length(mean.beta) )
l.alphaab <- numeric(length(mean.beta) )
for(i in 1:length(mean.beta)) { 
  u.alphaab[i] <- quantile(out$BUGSoutput$sims.list$BB[,i,2],probs=c(0.95) )
  l.alphaab[i] <- quantile(out$BUGSoutput$sims.list$BB[,i,2],probs=c(0.05) )
}
########
...
upper.CIAabc <- apply(est.lineAabc, 2, quantile, probs=c(0.95) )
lower.CIAabc <- apply(est.lineAabc, 2, quantile, probs=c(0.05) )

## Grab 90% CIs for beta's
u.alphaabc <- numeric(length(mean.beta) )
l.alphaabc <- numeric(length(mean.beta) )
for(i in 1:length(mean.beta)) { 
  u.alphaabc[i] <- quantile(out$BUGSoutput$sims.list$BB[,i,2],probs=c(0.95) )
  l.alphaabc[i] <- quantile(out$BUGSoutput$sims.list$BB[,i,2],probs=c(0.05) )
}
########
...
layout.show(nf)
par(mar=c(0.2,0.1,0.1,0.1),oma=c(1.3,2.2,0,1),mai=c(0.30,0.1,0.05,0) )
...
mtext(y.label, line = 0.6, side = 2, cex = size.text, outer=T, at=0.5)
text(-1.3,1.9,'(C)')
box()
