...
mean.beta <- out$BUGSoutput$mean$BB[,2]
...
for(i in 1:out$BUGSoutput$n.sims ){
  for(t in 1:length(fake1) ){
    est.lineA[i,t] <- out$BUGSoutput$sims.list$mu.b[i] + out$BUGSoutput$sims.list$gamma1b[i] * fake1[t] 
      }
}
...
for(i in 1:length(mean.beta)) { 
  u.alpha[i] <- quantile(out$BUGSoutput$sims.list$BB[,i,2],probs=c(0.975) )
  l.alpha[i] <- quantile(out$BUGSoutput$sims.list$BB[,i,2],probs=c(0.025) )
}
########
...
for(i in 1:out$BUGSoutput$n.sims ){
  for(t in 1:length(fake1) ){
    est.lineAa[i,t] <- out$BUGSoutput$sims.list$mu.b[i] + out$BUGSoutput$sims.list$gamma1b2[i] * fake1a[t] 
  }
}
...
for(i in 1:length(mean.beta)) { 
  u.alphaa[i] <- quantile(out$BUGSoutput$sims.list$BB[,i,2],probs=c(0.975) )
  l.alphaa[i] <- quantile(out$BUGSoutput$sims.list$BB[,i,2],probs=c(0.025) )
}
########
...
for(i in 1:out$BUGSoutput$n.sims ){
  for(t in 1:length(fake1) ){
    est.lineAab[i,t] <- out$BUGSoutput$sims.list$mu.b[i] + out$BUGSoutput$sims.list$gamma1b3[i] * fake1ab[t] 
  }
}
...
for(i in 1:length(mean.beta)) { 
  u.alphaab[i] <- quantile(out$BUGSoutput$sims.list$BB[,i,2],probs=c(0.975) )
  l.alphaab[i] <- quantile(out$BUGSoutput$sims.list$BB[,i,2],probs=c(0.025) )
}
########
...
for(i in 1:out$BUGSoutput$n.sims ){
  for(t in 1:length(fake1) ){
    est.lineAabc[i,t] <- out$BUGSoutput$sims.list$mu.b[i] + out$BUGSoutput$sims.list$gamma1b4[i] * fake1abc[t] 
  }
}
...
for(i in 1:length(mean.beta)) { 
  u.alphaabc[i] <- quantile(out$BUGSoutput$sims.list$BB[,i,2],probs=c(0.975) )
  l.alphaabc[i] <- quantile(out$BUGSoutput$sims.list$BB[,i,2],probs=c(0.025) )
}
########
...
layout.show(nf)
par(mar=c(0.2,0.1,0.1,0.1),oma=c(1.0,1.5,0,1),mai=c(0.10,0.05,0.05,0) )
...
y.label <- 'Relationship (slope) between GDD and lake-specific recruitment success'
...
points(area, mean.beta,pch=16,cex=cexPoint)

# segments(x0=area, x1=area,
#          y0=l.alpha, y1=u.alpha, col='black',lwd=1)


lines(fake1,fit1, lwd = lwdline, col="blue", lty = ltyLine)
...
points(cond, mean.beta,pch=16,cex=cexPoint)

# segments(x0=cond, x1=cond,
#          y0=l.alphaa, y1=u.alphaa, col='black',lwd=1)


lines(fake1a,fit1a, lwd = lwdline, col="blue", lty = ltyLine)
...
points(bass, mean.beta,pch=16,cex=cexPoint)

# segments(x0=bass, x1=bass,
#          y0=l.alphaab, y1=u.alphaab, col='black',lwd=1)


lines(fake1ab,fit1ab, lwd = lwdline, col="blue", lty = ltyLine)

mtext(x.label3, line = 1.21, side = 1, cex = size.text)
mtext(y.label, line = 0.4, side = 2, cex = size.text, outer=T, at=0.5)
text(-1.3,2,'(C)')
box()
...
points(gdd2, mean.beta,pch=16,cex=cexPoint)

# segments(x0=gdd2, x1=gdd2,
#          y0=l.alphaabc, y1=u.alphaabc, col='black',lwd=1)


lines(fake1abc,fit1abc, lwd = lwdline, col="blue", lty = ltyLine)

mtext(x.label4, line = 1.21, side = 1, cex = size.text)
#mtext(y.label, line = 1.1, side = 2, cex = size.text)
text(-2.0,2,'(D)')
box()
